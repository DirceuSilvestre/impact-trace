...
class EnhancedASTDependencyVisitor(ast.NodeVisitor):
    """
    Visitor da AST otimizado para extração de dependências locais em Python.
    """
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if level > 0:
            resolved = self._resolve_relative_import(
                module_name, level, [a.name for a in node.names]
            )
            for dep in resolved:
                self._add_dependency(dep)
        else:
            resolved = self._resolve_from_import(
                module_name, [a.name for a in node.names]
            )
            for dep in resolved:
                self._add_dependency(dep)
    ...
    def _resolve_relative_import(
        ...
        for name in imported_names:
            ...
            if submodule_py.is_file():
                results.add(
                    submodule_py.relative_to(self.project_root).as_posix()
                )
            else:
                submodule_init = target_base / name / "__init__.py"
                if submodule_init.is_file():
                    results.add(
                        submodule_init.relative_to(
                            self.project_root
                        ).as_posix()
                    )
    ...
    def _resolve_from_import(
        self, module: str, imported_names: List[str]
    ) -> Set[str]:
        results: Set[str] = set()
        ...
        if module:
            ...
            for name in imported_names:
                sub_candidate = self.project_root.joinpath(
                    *mod_parts, f"{name}.py"
                )
                if sub_candidate.is_file():
                    results.add(
                        sub_candidate.relative_to(self.project_root).as_posix()
                    )
                else:
                    sub_init = self.project_root.joinpath(
                        *mod_parts, name, "__init__.py"
                    )
                    if sub_init.is_file():
                        results.add(
                            sub_init.relative_to(self.project_root).as_posix()
                        )
...
class PythonLanguageParser(BaseLanguageParser):
    """
    Parser poliglota de dependências AST para arquivos Python (.py).
    """
    ...
    def parse_file(
        ...
        try:
            ...
            file_data = {
                ...
                "type_checking_imports": sorted(
                    list(visitor.type_checking_imports)
                ),
            }
